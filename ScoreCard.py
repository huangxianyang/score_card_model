...
def _check_target_binary(y):
    ...
    ------------------------------
    Param
    y:二分类变量,like list or pd.Series or narray
    ------------------------------
    Return
...
def woe_trans(varnames, y, df):
    ...
    for var in varnames:
        ...
        woe_data = pd.concat([y,x_woe_trans], axis=1)
        woe_maps[var] = woe_map
        iv_values[var] = info_value

    return woe_data, woe_maps, iv_values

#特征权重
def feature_importance(X,y,n_estimators):
    """
    基于随机森林特征权重
    --------------------------
    parameter:
    ...
    rf=RandomForestClassifier(n_estimators=n_estimators,random_state=1234)#构建分类随机森林分类器
...
def plot_roc_curve(pred_y, y):
    ...
    ks_value = "KS = %.2f" % max(abs(tpr-fpr))
    plt.test(0.8,0.4,ks_value,bbox = dict(facecolor = "g",alpha = 0.7))
...
def creditCards(paramsEst,woe_maps,bin_maps, red_maps, basepoints=600,odds=60,PDO=20):
    """
    评分卡生成函数,好坏比odds 尽量与样本保持一致,保证模型稳定性
